...
**從一句需求，到 PRD、架構、UI/UX、程式碼、品質門、交付包——底座寫程式碼，UmaDev 管流程。**
...
UmaDev 是一個本地執行的確定性殼：它給你**已經登入的 AI 編碼底座**（Claude Code / Codex / OpenCode）套上一條**治理軌道**，把「AI 寫程式碼」約束成一個可檢查、可恢復、可稽核的交付流程。它不取代這些底座，也不販售模型 API——**腦子始終在底座**，UmaDev 只管流程、規則和證據。

1. **把你的需求拆成完整交付流程**：先釐清，再調研，再寫 PRD、架構、UI/UX，再實作前後端，最後跑品質門和交付。
2. **驅動你已經登入的 AI 編碼工具**：Claude Code / Codex / OpenCode 負責真正寫程式碼，UmaDev 負責讓它們照流程做事——一等支援恰好這三個底座；想覆蓋更多模型，是把底座路由到第三方／本地模型，那是底座自己的事，不是 UmaDev 加新驅動。
3. **把交付過程變成可檢查、可恢復、可稽核**：每個階段有產物，每次重要操作有證據，最後有 quality gate、compliance mapping 和 proof pack。

如果一般 AI 編碼工具像一位能力很強的工程師，那麼 UmaDev 更像圍著它的一圈**流程與審稿崗位**——產品經理、架構師、UI/UX 審稿人、技術負責人、QA、交付經理的檢查清單，而不是一個能取代工程師的全自動總監。最終判斷和程式碼仍來自底座，UmaDev 負責讓這個過程不跑偏、留下證據。

> 定位與成熟度：UmaDev 仍處於**早期、治理優先**階段，最適合在真實專案上驗證它的流程與門禁。它把價值押在「軌道」上，而不是聲稱已經能無人值守地全自動交付。
...
flowchart TB
    ...
    UI --> Director["umadev-agent<br/>流程引擎：階段、gate、狀態、品質門"]
...
離線模式不呼叫任何模型，也不存取網路。它**不是一個讓你選的產品形態**——產品永遠是「驅動你登入的底座」。離線範本只是沒有底座可用時的確定性兜底，適合快速看檔案結構、CI smoke test 和流程示範。離線產物只是範本（帶 TODO 佔位），不代表模型完成了真正開發；真正交付必須走底座，第一次啟動的底座選擇器也只列這三個底座，不會把離線當成一個選項。
...
> 小任務有輕量路徑：9 階段是面向「完整商業級交付」的主鏈，不是每個需求都得全程走完。用 `/kind`（全端／僅前端／僅後端／bugfix／重構）宣告任務類型後，UmaDev 會據此裁剪階段——bugfix、小改動不會強行拉你走 PRD／架構／UIUX 全套。
...
這些規則是一條**治理基線**，不是絕對真理——每條都可以在 `.umadev/rules.toml` 裡停用、按路徑排除或調參。它們的作用是給底座的產出兜底，而不是替你做最終的工程判斷。
...
| `umadev-agent` | 流程編排引擎 | runner、gate、狀態、品質門、交付包 |
